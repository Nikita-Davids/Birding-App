...
class BestRoute : Fragment(), OnMapReadyCallback {
    ...
    override fun onCreateView(
        ...
        val layout = inflater.inflate(R.layout.fragment_best_route, container, false)
        ...
        val btnGetDirections = layout.findViewById<Button>(R.id.btnGetDirections)

        //Set an OnClickListener for the "Get Directions" button(The IIE,2023)
        btnGetDirections.setOnClickListener {
            //Checking if a hotspot location has been selected(The IIE,2023)
            if (selectedHotspotLatLng != null) {
                googleMap.clear() // Clear the map to remove previous route(The IIE,2023)

                //Introducing a delay before drawing the route(The IIE,2023)
                Handler().postDelayed({
                    drawRouteToSelectedHotspot(selectedHotspotLatLng!!) //Calling function to draw the route(The IIE,2023)
                }, 1000) //Delaying for 1 second (adjust the delay duration as needed)(The IIE,2023)
            } else {
                //If no hotspot is selected, show a toast message(The IIE,2023)
                Toast.makeText(
                    requireContext(),
                    "Select a hotspot for directions",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    override fun onMapReady(map: GoogleMap) {
        ...
        googleMap.setOnMarkerClickListener { marker ->
            //Retrieving the selected hotspot based on the marker's title(The IIE,2023)
